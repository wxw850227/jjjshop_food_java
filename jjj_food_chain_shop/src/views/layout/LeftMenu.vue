<template>
  <div class="left-menu-wrapper">
    ...
    <div class="menu-wrapper">
      ...
      <div class="d-c-c">
        <span class="p-10-0 blue fb tc">{{
          userInfo.shopName || "点餐系统连锁总店"
        }}</span>
      </div>
        ...
        <ul v-if="active_menu != null">
          ...
          >
            <li
              :class="
                active_child == index
                  ? 'routre-link router-link-active'
                  : 'router-link'
              "
              @click="choseMenu(3, item, index)"
              v-if="item.isMenu == 1"
            >
  ...
  setup(props, { emit }) {
    ...
    const selectMenu = (to) => {
      ...
      nextTick(() => {
        bus_emit(
          "MenuName",
          (to.meta && to.meta.showMenuTitle) || to.meta.title
        );
      });
    };
