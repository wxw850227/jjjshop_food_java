<template>
  <div class="user">
    ...
    <div class="product-content">
      <div class="table-wrap">
        ...
        >
          ...
          <el-table-column prop="realName" label="所属角色">
          ...
          <el-table-column prop="user_name" label="所属门店" width="120">
            <template #default="scope">
          ...
          <el-table-column prop="user_name" label="门店状态">
            <template #default="scope">
              <div v-if="scope.row.supplier">
                <span v-if="scope.row.supplier.isDelete == 1" class="red"
                  >已删除</span
                >
          ...
          <el-table-column prop="isStatus" label="账号状态" width="80">
            <template #default="scope">
              <div
                v-if="
                  scope.row.isSuper < 1 ||
                  (scope.row.isSuper == 1 && scope.row.userType == 1)
                "
              >
                <el-switch
    ...
    <Edit
      :open="open_edit"
      :roleList="roleList"
      :shopUserId="curModel.shopUserId"
      @close="closeEdit"
    ></Edit>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getTableList() {
      ...
      AuthApi.userList(Params, true)
        .then((data) => {
          ...
          self.tableData = data.data.userList.records.map((user) => {
            return {
              ...user,
              isSuper: user.isSuper ? 1 : 0,
            };
          });
          console.log(
            "issuper",
            self.tableData.map((item) => item.isSuper)
          );
          self.totalDataNumber = data.data.userList.total;
        })
        .catch((error) => {});
    },
    ...
    isShowFunc(e) {
      let self = this;
      AuthApi.setStatus({ shopUserId: e.shopUserId, status: e.isStatus }, true)
        .then((data) => {
          if (data.code == 1) {
            ElMessage({
              message: data.msg,
