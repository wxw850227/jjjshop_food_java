    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-table size="small" :data="tableData" border style="width: 100%" v-loading="loading" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="attributeId" label="ID" sortable></el-table-column>
          <el-table-column prop="attributeName" label="属性名" width="400px"></el-table-column>
          <el-table-column prop="attributeValueList" label="属性值">
            <template  #default="scope">
             {{attrjoin(scope.row.attributeValueList)}}
            </template>
          </el-table-column>
          <el-table-column fixed="right" label="操作" width="120">
            <template  #default="scope">
              <el-button @click="editClick(scope.row)" type="text" size="small" v-auth="'/product/expand/attr/edit'" >编辑</el-button>
              <el-button @click="deleteClick(scope.row.attributeId)" type="text" size="small" v-auth="'/product/expand/attr/delete'" >删除</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>
  ...
  methods: {
    ...
    deleteBatch(){
      let self = this;
       ...
       let attributeId=arr.join(',');
      ElMessageBox.confirm('删除后不可恢复，确认删除吗?', '提示', {
          type: 'warning'
        })
        .then(() => {
          PorductApi.deleteAttributes({
