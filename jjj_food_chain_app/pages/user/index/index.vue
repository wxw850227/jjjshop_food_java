<template>
	<view class="user-index pb60" :data-theme='theme()' :class="theme() || ''">
		...
		<view class="menu-wrap">
			<view class="order-top fb">服务功能</view>
			<view class="group-bd f-w">
				<template v-for="(item, index) in menus" :key="index">
					<view class="item" v-if="item.isShow" @click="jumpPage(item.path)">
						<view class="icon-round d-c-c">
							<image class="icon-round" :src="item.icon" mode="aspectFit"></image>
						</view>
	...
	export default {
		...
		methods: {
			...
			getData() {
				...
				self._postBody('user/index/detail', {
					url: self.urldata,
				}, function(res) {
