		...
		<view v-if="listData.length>0" class="pbenv">
			<view class="address-list bg-white">
				<view class="address p-30-0 border-b-d" v-for="(item,index) in listData" :key="index">
					...
					<view class="d-b-c">
						<view class="gray9 d-b-c"><text>距离：{{item.distance}}km</text><text v-if="item.status==0"
								class="red">超出配送距离</text></view>
						...
						<view class="d-s-c">
							...
							<view class="icon-box plus d-c-c ml30 mr40" @click="editAddress(item.addressId)">
					...
					<view class="radio d-s-c">
						<radio style="transform:scale(0.6)" :color='getThemeColor()' :value="item.addressId+''"
							:checked="defaultId==item.addressId+''" @click="radioChange(item.addressId)" />
...
<script>
	export default {
		...
		methods: {
			...
			editAddress(e) {
				uni.navigateTo({
					url: '/pages/user/address/edit/edit?addressId=' + e,
				});
			},
