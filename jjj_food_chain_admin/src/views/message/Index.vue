<template>
  <div class="product">
    ...
    <div class="common-level-rail">
      <el-button type="primary" @click="addClick">添加消息</el-button>
    </div>
        ...
        >
          ...
          <el-table-column prop="messageTo" label="通知对象">
            <template #default="scope">
              <el-tag v-if="scope.row.messageTo == 10">会员</el-tag>
              <el-tag v-if="scope.row.messageTo == 20">商家</el-tag>
              <el-tag v-if="scope.row.messageTo == 30">供应商</el-tag>
            </template>
          </el-table-column>
          <el-table-column prop="messageType" label="消息类型">
            <template #default="scope">
              <el-tag v-if="scope.row.messageType == 10">订单</el-tag>
              <el-tag v-if="scope.row.messageType == 20">分销</el-tag>
              <el-tag v-if="scope.row.messageType == 30">通知</el-tag>
            </template>
          </el-table-column>
...
export default {
  ...
  methods: {
    ...
    editClick(item) {
      ...
      this.messageModel.messageTo = this.messageModel.messageTo;
      this.messageModel.messageType = this.messageModel.messageType;
