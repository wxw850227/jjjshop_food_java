...
public class AccessToken {
    ...
    public AccessToken(String appId, String appCert, int expire) {
        ...
        this.appId = appId;
        this.expire = expire;
        this.issueTs = AgoraUtils.getTimestamp();
        this.salt = AgoraUtils.randomInt();
    }
    ...
    public String build() throws Exception {
        if (!AgoraUtils.isUUID(this.appId) || !AgoraUtils.isUUID(this.appCert)) {
            return "";
        }
        ...
        return getVersion() + AgoraUtils.base64Encode(AgoraUtils.compress(bufferContent.asBytes()));
    }
    ...
    public boolean parse(String token) {
        if (!getVersion().equals(token.substring(0, AgoraUtils.VERSION_LENGTH))) {
            return false;
        }
        byte[] data = AgoraUtils.decompress(AgoraUtils.base64Decode(token.substring(AgoraUtils.VERSION_LENGTH)));
