...
/**
 * ClassName: RtcTokenBuildUtil
 * Package: net.jjjshop.common.util
 * Description:
 *
 * @Author cheng
 * @Create 2023/4/6 16:29
 */
@Slf4j
@Component
public class RtcTokenBuildUtil {
    public enum Role {
        ROLE_PUBLISHER(1),
        ROLE_SUBSCRIBER(2),
        ;
    ...
    public String getTokenWithUid(String appId, String appCertificate, String channelName,boolean isCaster, int uid) {
        Role role = isCaster?Role.ROLE_PUBLISHER:Role.ROLE_SUBSCRIBER;
        int expireTime = 3600;
        int privilegeTime = (int) (new Date().getTime() / 1000 + expireTime);
        return buildTokenWithUserAccount(appId, appCertificate, channelName, AccessToken.getUidStr(uid), role, expireTime, privilegeTime);
    }
