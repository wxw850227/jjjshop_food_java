...
public class UserLoginRedisServiceImpl implements UserLoginRedisService {
    ...
    public void cacheLoginInfo(JwtToken jwtToken, LoginUserVo loginUserVo) {
        ...
        // 1. tokenMd5:jwtTokenRedisVo
        String loginTokenRedisKey = String.format(CommonRedisKey.USER_LOGIN_TOKEN, tokenMd5);
        redisTemplate.opsForValue().set(loginTokenRedisKey, jwtTokenRedisVo, expireDuration);
        // 2. username:loginSysUserRedisVo
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.USER_LOGIN_USER, username), loginUserRedisVo, expireDuration);
        // 3. salt hash,方便获取盐值鉴权
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.USER_LOGIN_SALT, username), salt, expireDuration);
        // 4. login user token
        redisTemplate.opsForValue().set(String.format(CommonRedisKey.USER_LOGIN_USER_TOKEN, username, tokenMd5), loginTokenRedisKey, expireDuration);
    }
    ...
    public String getSalt(String username) {
        ...
        String salt = (String) redisTemplate.opsForValue().get(String.format(CommonRedisKey.USER_LOGIN_SALT, username));
        return salt;
    }
