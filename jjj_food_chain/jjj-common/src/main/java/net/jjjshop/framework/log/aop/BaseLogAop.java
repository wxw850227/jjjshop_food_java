...
package net.jjjshop.framework.log.aop;
...
import net.jjjshop.config.properties.SpringBootJjjAopProperties;
import net.jjjshop.framework.common.api.ApiCode;
import net.jjjshop.framework.common.api.ApiResult;
import net.jjjshop.framework.common.bean.ClientInfo;
import net.jjjshop.framework.ip.service.IpAddressService;
...
import net.jjjshop.framework.util.*;
...
public abstract class BaseLogAop {
    ...
    protected void saveShopOperationLog(RequestInfo requestInfo, OperationLogInfo operationLogInfo, Object result, Exception exception) {
        try {
            ...
            // 操作日志
            ShopOptLog shopOptLog = new ShopOptLog();
            // 设置操作日志信息
            ...
            // 设置请求参数信息
            if (requestInfo != null) {
                shopOptLog.setIp(requestInfo.getIp())
                        .setUrl(requestInfo.getPath())
                        .setRequestType(requestInfo.getRequestMethod());

                // 设置参数字符串
                shopOptLog.setContent(Jackson.toJsonStringNonNull(requestInfo.getParam()));
                // User-Agent
                String userAgent = requestInfo.getUserAgent();
                if (StringUtils.isNotBlank(userAgent)) {
                    shopOptLog.setAgent(StringUtils.substring(userAgent, ZERO, MAX_LENGTH));
                }
                ClientInfo clientInfo = ClientInfoUtil.get(requestInfo.getUserAgent());
                if (clientInfo != null) {
                    shopOptLog.setBrowser(clientInfo.getBrowserName());
                }
            }

            // 设置当前登录信息
            shopOptLog.setShopUserId(shopUserId.intValue());
            // 保存日志到数据库
            shopOptLogService.save(shopOptLog);
        } catch (Exception e) {
        ...
        try {
            ...
            if (flag && result != null && result instanceof ApiResult) {
                ...
                if (apiResult.getCode() == 1) {
                    ...
                    shopLoginLog.setResult(optName);
                    shopLoginLog.setIp(requestInfo.getIp());
                    if ("登录成功".equals(optName)) {
                        ...
                        shopLoginLog.setUsername(paramObject.getString("username"));
                        shopLoginLog.setAppId(loginShopUserVo.getAppId());
                    }else {
                        String username = JwtUtil.getUsername(requestInfo.getToken());
                        shopLoginLog.setUsername(username);
                    }
                    // 保存登录日志
                    shopLoginLogService.save(shopLoginLog);
                }
            }
        } catch (Exception e) {
