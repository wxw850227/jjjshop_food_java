...
package net.jjjshop.shop.controller.product.takeaway;
...
@Slf4j
@Api(value = "index", tags = {"外卖商品管理"})
@RestController
@RequestMapping("/shop/product/takeaway/product")
public class ProductController {
    @Autowired
    private ProductService productService;

    @RequestMapping(value = "/index", method = RequestMethod.POST)
    @RequiresPermissions("/product/takeaway/product/index")
    ...
    public ApiResult<Map<String,Object>> index(@Validated @RequestBody ProductPageParam productPageParam) throws Exception{
        //0外卖1店内
        productPageParam.setProductType(0);
        return ApiResult.ok(productService.getList(productPageParam));
    }

    @RequestMapping(value = "/add", method = RequestMethod.GET)
    @RequiresPermissions("/product/takeaway/product/add")
    @OperationLog(name = "add")
    @ApiOperation(value = "add", response = String.class)
    public ApiResult<Map<String, Object>> toAdd() {
        //type 0外卖1店内
        return ApiResult.ok(productService.getBaseData(0, "add",0));
    }
    ...
    @RequiresPermissions("/product/takeaway/product/edit")
    @OperationLog(name = "edit")
    @ApiOperation(value = "edit", response = String.class)
    public ApiResult<Map<String, Object>> toEdit(Integer productId, String scene) {
        return ApiResult.ok(productService.getBaseData(productId, scene,0));
    }
    ...
    @RequiresPermissions("/product/takeaway/product/state")
    ...
    @RequiresPermissions("/product/takeaway/product/delete")
